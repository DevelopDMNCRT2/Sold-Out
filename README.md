...
- **Rediseño Premium de Boleto (PDF & jsPDF):** Rediseñamos estéticamente el boleto PDF incorporando una franja roja lateral redondeada con el logo "SOLD OUT." vertical, cuadrícula de datos 2x2 para asistente/acceso, portada de evento en formato 100% cuadrado (1:1), muescas semicirculares de corte y un banner gris de instrucción en el talón de acceso.
- **Control de Stock Estricto:** Añadimos un bloqueo interactivo en el frontend para no permitir seleccionar más boletos del stock restante disponible en NeonDB. En el backend, implementamos una validación en la API de órdenes que rechaza el checkout si supera la disponibilidad del evento.
- **Integración de Mercado Pago (Checkout Bricks):** Agregamos soporte para cobros reales con tarjeta en el checkout integrando el *Card Payment Brick* de Mercado Pago de forma 100% embebida. El Brick cuenta con carga ultrarrápida (preload en el HTML) y adapta su color de fondo automáticamente al tema del sitio (blanco en diurno y negro en nocturno) usando un MutationObserver, manteniendo siempre el botón de cobro en rojo corporativo de SoldOut.
- **Alineación de Tabla y Métricas (Admin):** Corregimos el desfase de columnas (Total, Fecha, Estado) en el listado de ventas de eventos del panel administrativo. Truncamos los folios UUID a 8 caracteres con tooltip hover, agregamos insignias de color para estados de orden (Pagado/Pendiente/Rechazado) y habilitamos los botones de detalle excluyéndolos del fieldset disabled en modo lectura.
- **Distribución Limpia de Detalles de Evento (Cliente):** Eliminamos la sección de croquis/mapa del recinto del cliente y rediseñamos la vista de detalles en un grid asimétrico limpio, colocando el póster del evento y la tarjeta de horarios lado a lado, y dejando el módulo de compra de entradas en una columna sticky flotante independiente.
